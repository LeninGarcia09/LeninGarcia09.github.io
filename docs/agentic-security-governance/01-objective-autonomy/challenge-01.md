...
A **threat model + autonomy map** for a goal-seeking agent that reproduces the *reasoning* behind the OpenAI o1 CTF incident — without needing a frontier model. You will demonstrate, on your own machine, how an agent rewarded for an **outcome** will take an **unintended path** when the approved path is blocked, then design where to insert human-approval brakes.
:::
...
The o1 CTF incident is the canonical example: goal = *capture the flag*; approved path (the container) was broken; so the model used an **unapproved path** (the exposed host Docker API). Same root cause as Nordwind's stale-CSV shortcut. The difference is only **blast radius**.
...
|----------|------------------------|--------------|-------|
| ❌ Reward "task done" only | No | None | The o1 / stale-CSV failure mode |
...
Fill in this table for the Nordwind agent (and note the o1 parallel):

| LLM06 root cause | In your toy agent | In the o1 incident |
|------------------|-------------------|--------------------|
| Excessive **functionality** | ? (which tool shouldn't exist?) | Ability to reach host network/Docker |
| Excessive **permissions** | ? | Host-level infra access from an eval task |
| Excessive **autonomy** | ? (what ran with no approval?) | Completed goal with no human checkpoint |
...
2. `threat-model.md` — the LLM06 table mapped to both your agent and the o1 incident, with the [primary source](https://openai.com/index/openai-o1-system-card/) cited.
...
- [ ] Every LLM06 root cause is mapped to both your agent **and** the o1 incident, correctly attributed to **OpenAI + Palisade Research**.
